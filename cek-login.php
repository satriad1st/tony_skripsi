...
if ($num_rows > 0) {
    $rows = $db->db_fetch_array($result);
        ...
        $_SESSION['apriori_resep_id'] = $rows['id']; // mengisi session
        $_SESSION['apriori_resep_username'] = $rows['username'];
        $_SESSION['apriori_resep_level'] = $rows['level'];

        $level_name = ($_SESSION['apriori_resep_level']==1)?"admin":"kepala";
        $_SESSION['apriori_resep_level_name'] = $level_name;
        $_SESSION['apriori_resep_key'] = sha1(date("Y-m-d H:i:s") . $rows['id']);
        $_SESSION['apriori_resep_last_login'] = date("d-m-Y H:i:s");
